...
def SRsectorial(l, s, n, m, k, rpq):
    ...
    if issect((l, s, n, m)):
        if (l < 0) | (n < 0):
            ...
            return (bnm(l, -s) * SRsectorial(l - 1, s - 1, m - 1, m - 1, k, rpq) - bnm(l + 1, s - 1) * SRsectorial(
                l + 1, s - 1, m - 1, m - 1, k, rpq)) / bnm(m, -m)

        elif m <= 0:
            #            print("m<0 used",l,s,n,m,"these needed:",l - 1, s + 1, np.abs(m+1), m+1,"     ",l+1, s+1, -(m+1), m+1)
            return (bnm(l, s) * SRsectorial(l - 1, s + 1, np.abs(m + 1), m + 1, k, rpq) - bnm(l + 1,
                                                                                              -s - 1) * SRsectorial(
                l + 1, s + 1, -(m + 1), m + 1, k, rpq)) / bnm(-m, m)
        else:
            print("line188")
